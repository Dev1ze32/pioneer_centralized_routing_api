...
---------
    GET    /api/logs
        List audit log entries. Supports filtering and pagination.
        Admin only.

    DELETE /api/logs/cleanup
        Purge entries older than 90 days (default) or a custom number
        of days supplied via ?days=N.
        Admin only.
"""
...
# ── Helper: single decorator that enforces admin-only ─────────────────────────
...
def list_logs():
    ...
    for row in rows:
        entry = dict(row)
        # Parse JSONB extra back to dict if present
...
def cleanup_logs():
    ...
    log_action(
        ...
        description=f"{g.current_user.username} deleted {deleted} audit log "
